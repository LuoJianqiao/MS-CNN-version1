...
###########multi-net####################
class multi_alex(nn.Module):
    def __init__(self,model,num_class=45):
        super(multi_alex,self).__init__()
...
model1=model1.cuda()
model2=multi_alex(model1,num_classes)
model2=model2.cuda()
#############model######################

############criteria ,optimizer, learn rate####################
criterion = nn.CrossEntropyLoss()
opt = optim.SGD([{'params':model2.classifier.parameters(),'lr':0.01},
                 {'params':model2.distribution.parameters(),'lr':0.01},
                 {'params':model2.feature.parameters()}]
                , lr=0.001, momentum=0.9,weight_decay=5e-4)

lr_v=lr_scheduler.ReduceLROnPlateau(opt,mode='min',factor=0.2,patience=3,verbose=True)
############criteria ,optimizer, learn rate####################

############para initialise####################
wts_val=copy.deepcopy(model2.state_dict())
num_epochs=100
lamda=1.6#the optimal lamda in different cases is given in the paper at Table 6
############para initialise####################
